...
State setSystemState(float systemPressure)
{
	if (systemPressure < 10)
	{
		return State::TOO_LITTLE_PRESSURE;
	}
	else if (systemPressure > 20)
	{
...
int main()
{
	...
	while (system_pressure > 0)
	{
		...
		State currentState = setSystemState(system_pressure);
		
		// Choose Action
		// TODO: Store a local variable that explains what action the AI will perform
		float bestValue = Q[currentState][0];

		for (int i = 0; i < ACTION_NUM; ++i)
		{
			if (Q[currentState][i] > bestValue)
			{
				bestValue = Q[currentState][i];
			}
		}
